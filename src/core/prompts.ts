...
const roomModePromptTemplates: RoomModePromptTemplate[] = [
  {
    ...
    roomFields: [
      ...
      field("interactionPace", "Interaction pace", "How often roles should speak.", "Use short, distinct turns. Roles may speak, stay silent, observe, react, or wait when they have no fresh pressure. Do not make multiple roles repeat the same idea or speak when another role already handled the point."),
...
function commonRoleFields(): PromptTemplateField[] {
  return [
    ...
    field("speakingActivity", "Dampers", "When this role should reduce output.", "Fill in dampers if needed. Short replies, partial agreement, silence, observation, and letting another role carry the point are valid when they fit the moment."),
...
export function compileRoomRulesPrompt(fields: RoomRulesFields, mode: RoomContextPanelMode = fields.mode): string {
  ...
  return compilePromptFromFields("Room Rules", template.name, template.roomFields, fields.values, [
    ...
    "Roles may speak, stay silent, observe, challenge, add one useful angle, or wait when they have no fresh pressure.",
