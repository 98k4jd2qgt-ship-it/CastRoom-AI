...
const roomModePromptTemplates: RoomModePromptTemplate[] = [
  {
    ...
    name: "Casual",
    roomFields: [
      field("commonTopic", "Common topic", "What this room usually talks about.", "This room is for natural conversation. Roles may respond, ask questions, react, or lightly extend the topic, but they should not force a plot, debate, lesson, or planning structure unless the user asks."),
      ...
      field("interactionPace", "Interaction pace", "How often roles should speak.", "Use short, distinct turns. Do not make multiple roles repeat the same idea or speak when another role already handled the point."),
      field("modeTechnique", "Mode technique", "Small operating guidance for this mode.", "Roles should add new information, answer directly, or ask one useful question. Do not treat greetings, jokes, or passing comments as long-term facts."),
      field("simulationGoal", "Room Flow goal", "What autoplay should try to create.", "Wait for user direction when there is no visible next step. Do not invent a premise or convert casual chat into a structured scenario."),
      field("stopConditions", "Stop conditions", "When the room should pause.", "Pause on repetition, unavailable model, direct player choice, unclear direction, or lack of a visible next step."),
    ],
  ...
  },
  {
    ...
    roomFields: [
      ...
      field("modeTechnique", "Mode technique", "Small operating guidance for this mode.", "Roles should act, question, react, investigate, negotiate, or resist naturally. Do not narrate internal system logic or Director planning, and do not reveal hidden facts, private motives, or private channel information."),
  ...
  },
  {
    ...
    roomFields: [
      ...
      field("modeTechnique", "Mode technique", "Small operating guidance for this mode.", "Work through clues, hypotheses, contradictions, and visibility boundaries. Reveal private facts only when the room state makes them visible."),
  ...
  },
  {
    ...
    roomFields: [
      ...
      field("modeTechnique", "Mode technique", "Small operating guidance for this mode.", "Use the assigned side and speaking position. Give one argument, rebuttal, or summary per turn; do not repeat the user's long debate setup or rules."),
  ...
  },
  {
    ...
    roomFields: [
      ...
      field("modeTechnique", "Mode technique", "Small operating guidance for this mode.", "Roles should avoid repeating the same explanation. Corrections should be clear, specific, and tied to the user's answer."),
  ...
  },
  {
    ...
    roomFields: [
      ...
      field("discussionMode", "Discussion mode", "How the room should think.", "Different roles should contribute different angles such as strategy, risk, execution, counterexample, or user impact."),
  ...
  },
  {
    ...
    roomFields: [
      ...
      field("collaborationStyle", "Collaboration style", "How the team should work together.", "A huddle should be short and complete: goal, risk, boundary, assigned public speaker, and public-safe point."),
...
function commonDirectorFields(mode: RoomContextPanelMode, style: string): PromptTemplateField[] {
  return [
    field(
      ...
      [
        "You are the current room's background host, pacing controller, fact ledger, and visibility gatekeeper.",
    ...
    field(
      ...
      [
        ...
        "Write public output as immersive hosting or narration.",
    ...
    field(
      ...
      [
        "Role assignments, next speaker selection, faction strategy, debate position tasks, and action goals must use runtime private directives.",
    ...
    field(
      ...
      [
        ...
        "Changes to scene facts, item ownership, locks, secrets, harm, victory, or continuity must enter the room fact ledger.",
...
function commonRoleFields(): PromptTemplateField[] {
  return [
    field("roomIdentity", "Room identity", "Who this role is in this room.", "Fill in this role's temporary room identity if needed. If empty, use the character pack base prompt."),
    field("roomGoal", "Room goal", "What this role wants in this room.", "Fill in this role's room-only goal if needed. If empty, respond to the current visible context."),
    field("factionStance", "Faction / stance", "Team, side, or position in this room.", "Fill in a team, side, or stance if needed. If empty, use the room assignment or stay neutral."),
    field("speakingActivity", "Speaking activity", "How active this role should be.", "Fill in speaking activity if needed. If empty, speak only when relevant and listen when another role has the floor."),
    field("visibleKnowledge", "Visible knowledge note", "What this role knows in this room.", "Use only public facts, this role's visible private notes, its faction channel memory, and facts visible to this role. Do not repeat long user instructions or room setup text."),
    field("toneAddon", "Room tone note", "Temporary tone only for this room.", "Fill in room-only tone notes if needed. If empty, keep the character pack base prompt."),
  ];
}
...
export function compileDirectorRulesPrompt(fields: DirectorRulesFields, mode: RoomContextPanelMode = fields.mode): string {
  ...
  return compilePromptFromFields("Director Rules", template.name, template.directorFields, fields.values, [
    ...
    "Public Director text should be immersive; keep technical judgement state in Room Inspector, not the timeline.",
